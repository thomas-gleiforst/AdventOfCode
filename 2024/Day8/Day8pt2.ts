...
export const Part2 = () => {
    ...
    const posSet: {row: number, col: number}[] = [...allSignals];
    ...
    Object.entries(signalMap).forEach((entry) => {
        ...
        positions.forEach((pos) => {
            ...
            positions.forEach((otherPos) => {
                ...
                let newPos = {row, col}
                while (
                    newPos.col >= 0 && newPos.col < lines[0].length && 
                    newPos.row >= 0 && newPos.row < lines.length
                ) {
                    newPos = {row: newPos.row + rowDiff, col: newPos.col + colDiff}
                    if (
                        newPos.col >= 0 && newPos.col < lines[0].length && 
                        newPos.row >= 0 && newPos.row < lines.length &&
                        !findPos(newPos, posSet)
                    ) {
                        posSet.push(newPos)
